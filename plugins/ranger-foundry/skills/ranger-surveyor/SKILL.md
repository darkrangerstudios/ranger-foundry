...
description: "Surveyor resolves metric definitions, entity identity, aliases and retrieval meaning. Use when ambiguous terms, incompatible units or conflicting sources could change an answer or data operation; not for general web collection or storing session memory."
---
...
Map these semantic fields into the approved repository record format. If Scribe
is available and its record envelope is accepted for this environment, load its
record-format reference and reuse its version, provenance, status and supersession
...
This skill works on its own. When a peer materially helps, resolve it through the
host's actual skill catalog, load its entrypoint, and apply its method to a bounded
subtask. Scribe can prepare an authorized durable representation; Scout can gather
missing source evidence; Bounty Hunter can trace the cause of definition drift;
Kestrel can independently review a consequential change. Marshal retains any
overall job he is coordinating. These are examples, not a restrictive peer list.

Use the actual skills `ranger-scribe`, `ranger-scout`,
`ranger-bounty-hunter`, `ranger-kestrel` or `ranger-marshal` when available.
Pass the scope, artifact revision, allowed actions, relevant evidence, remaining
time/attempt/spend and delegation limits, and expected return. Peers share those
limits and return to the caller; they cannot reset budgets or create another owner.
Stop repeated delegation of the same unresolved question without new evidence.

Validate the returned artifact, scope, evidence and consumed/remaining budget.
When a peer is unavailable, use only the method that can be supported inline and
name the gap; never invent a peer result. Skill loading is not a running agent,
message delivery or an independent review. Dispatch a real separate context or
human when an independent gate applies.

## Return

Give the resolved meaning or unresolved alternatives, scope and source revision,
entity/key and comparability decisions, source-backed evidence versus inference,
consumer impact, any proposed mapping or change, and the exact missing evidence
or next action. Include authorized changes actually made and budget accounting
where observable. Unknown usage is unknown. Preserve every open review and
release gate; a well-grounded interpretation is not permission to deploy it.
