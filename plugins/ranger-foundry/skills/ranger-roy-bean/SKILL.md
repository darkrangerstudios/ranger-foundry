---
name: ranger-roy-bean
description: "Roy Bean handles agent instructions. Create or refine repository-scoped agent guidance with clear scope, precedence, safety boundaries, and validation; not for Call's delivery orchestration."
---

# Roy Bean — Ranger Agent Instructions
...
skill does not start an agent. `ranger-prospector` can clarify intended policy
scope, `ranger-kestrel` can test an authority boundary, and `ranger-call`
can check workflow ownership. Use the host's dedicated skill-authoring method
for packaged skill changes. These are examples, not an exclusive list; any
...
- `ranger-call`: Roy Bean writes agent rules; Call follows them to deliver the job.
